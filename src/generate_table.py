...
def generate_summary_table(seqs):
	...
	t.add_column("File name", ["Total Reads", "Error Free", "Paired Reads", "Singletons", "Error Reads", "Paired", "Direction", "Interleafed", "Wrapped"])
	for seq in seqs:
		num_paired = str(len(seq) - 1) if seq[0]["paired"] else str(0)
		t.add_column(''.join(os.path.splitext(os.path.basename(seq[0]["filename"]))), 
			[str(len(seq) - 1 + seq[0]["error_reads"] + seq[0]["singletons"]), 
			str(len(seq) - 1 + seq[0]["singletons"]),
			num_paired,
			str(seq[0]["singletons"]), str(seq[0]["error_reads"]), 
			str(seq[0]["paired"]), str(seq[0]["direction"]), str(seq[0]["leafed"]), 
			bool(seq[0]["wrapped"])])
	t.align = 'l'
	print(t)
